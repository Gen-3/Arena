...
public class LightningSO : MagicBaseSO
{
    public override void Execute(Battler user, Battler target)
    {
        ...
        Debug.Log($"{user.name}のライトニングで{target.name}に{damage}のダメージ(最小値は{(user.men - target.men) / 4}、最大値は{(user.men - target.men) / 3 + 10})");
        Pronpter.instance.UpdateConsole($"{user.unitName}のライトニングで{target.unitName}に{damage}のダメージ");
